...
// CHART: createECharts3DFilterOperations
...
// ============================================================
        ...
        function createECharts3DFilterOperations(requests, grouping) {
            ...
            // For stacked bar3D: [x (time), y (collection), z (value), stackGroup, filterType]
            const equalData = [];
            const notEqualData = [];
...
// CHART: expandECharts3DFilterOperations
...
// ============================================================
        ...
        function expandECharts3DFilterOperations() {
            ...
            function updateChart() {
                ...
                // Filter data based on visibility states
                // Offset blue bars slightly left (-0.2) and red bars slightly right (+0.2) for side-by-side display
                const filteredEqualData = filterTypeVisibilityState.equal 
                    ? equalData.filter(d => visibilityState[d.collection]).map(d => ({
                        ...d,
                        value: [d.value[0] - 0.2, d.value[1], useLogScale ? Math.log10(d.actualCount + 1) : d.actualCount]
                    }))
                    : [];
                    
                const filteredNotEqualData = filterTypeVisibilityState.notEqual 
                ...
                if (request.plan) {
                    try {
                        ...
                        const extractOperatorThroughput = (node) => {
                            ...
                            // Handle IntersectScan's scans array (Issue #237)
                            if (node.scans && Array.isArray(node.scans)) {
                                node.scans.forEach(scan => extractOperatorThroughput(scan));
                            }
                        };
...
// Expose ChartTimeUtils globally
window.ChartTimeUtils = ChartTimeUtils;
window.getTimeGrouping = () => ChartTimeUtils.getTimeGrouping();
window.getOptimalTimeUnit = (r) => ChartTimeUtils.getOptimalTimeUnit(r);
window.roundTimestamp = (t, g, r) => ChartTimeUtils.roundTimestamp(t, g, r);
window.getTimelineBucketsFromRequests = (r, g) => ChartTimeUtils.getTimelineBucketsFromRequests(r, g);
