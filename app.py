...
from flask import jsonify, request, send_file, send_from_directory, redirect
...
def _load_server_config() -> dict:
    ...
    Load server-side config from the first existing JSON file in:
      1. $APP_CONFIG_FILE
      2. ./app/config.json     (when running from repo root)
      3. ./config.json         (when running from /app inside container)
      4. ./app/config.default.json
    ...
    candidates.extend([
        os.path.join(here, 'app', 'config.json'),
        os.path.join(here, 'config.json'),
        os.path.join(here, 'app', 'config.default.json'),
...
# Server Edition v5.0.0: serve the analyzer (with the new "App Data Storage"
# settings tab) at "/" instead of the v4 Cloudflare marketing landing page.
# The analyzer lives at /app/app/index.html inside the container; redirecting
# preserves its relative asset paths (assets/js/..., assets/css/...) which
# resolve to /app/app/assets/* on disk via app_base.serve_static.
def _server_edition_index():
    return redirect('/app/index.html', code=302)

_override_route('/', _server_edition_index, methods=['GET'])
