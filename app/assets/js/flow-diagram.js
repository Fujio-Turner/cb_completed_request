...
// ============================================================
        ...
        function buildIndexQueryFlow_Force(requests = null) {
            // Use provided requests or fall back to original requests
            const requestsToUse = requests || originalRequests;

            Logger.debug(`[Flow] buildIndexQueryFlow_Force called with ${requestsToUse?.length || 0} requests`);

            if (!requestsToUse || requestsToUse.length === 0) {
                Logger.warn('[Flow] No requests to process');
                return;
            }
            ...
            // FORCE RENDER: Skip tab visibility check to ensure diagram renders
            // The performance impact is minimal since this is only called on lazy load or filter change
            const activeTabIndex = $("#tabs").tabs("option", "active");
            Logger.debug(`[Flow] Active tab: ${activeTabIndex}, Rendering diagram immediately`);
                ...
                if (request.plan) {
                    ...
                    if (planObj && planObj["#operator"]) {
                        ...
                        if (bucketScopeCollection === "_default._default._default" && statement.includes("conversations")) {
                            Logger.warn('[flow]', "[BSC DEBUG] Failed to parse BSC from statement containing 'conversations':", statement.substring(0, 200));
                        }
            ...
            Logger.debug('[flow]', `[BUILD DEBUG] Total indexes before filter: ${allIndexesArray.length}`);
            ...
            const sortedIndexes = allIndexesArray
                .filter(index => {
                    ...
                    if (!hasUsage && !hasStats) {
                        Logger.debug('[flow]', `[BUILD DEBUG] Filtered OUT ${hashCompositeKey(index.name + '::' + index.bucketScopeCollection)}: no usage or stats`);
                    }
            ...
            Logger.debug('[flow]', `[BUILD DEBUG] Total sortedIndexes passed to renderIndexQueryFlow: ${sortedIndexes.length}`);
            const joinRelatedIndexes = sortedIndexes.filter(idx => 
                idx.bucketScopeCollection && idx.bucketScopeCollection.split('.')[0] === 'HASH6'
            );
            Logger.debug('[flow]', `[BUILD DEBUG] Indexes for HASH6 bucket:`, joinRelatedIndexes.map(idx => 
...
export {
    buildIndexQueryFlow_Force as buildIndexQueryFlow
};

// Expose globally for backward compatibility
window.buildIndexQueryFlow = buildIndexQueryFlow_Force;

Logger.info('[flow]', '✅ flow-diagram.js module loaded');
